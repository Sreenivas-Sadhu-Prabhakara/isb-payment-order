...
```bash
mvn -B verify
docker build -t isb/isb-payment-order:0.1.0 .
```
...
Exposed through the platform Gateway at path prefix `/isb-payment-order` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
